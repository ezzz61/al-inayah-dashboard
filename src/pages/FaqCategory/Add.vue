    ...
    <b-container fluid>
      ...
      <b-row class="mt-5 justify-content-center">
        <b-col cols="6" md="6" class="my-1">
          <card>
            <div>
              <h1 class="text-center">Add faq category</h1>
...
export default {
  ...
  data() {
    return {
      ...
      options_Faq: [{ value: null, text: "Please select an floor" }],
      selected: null,
      arr_criteria: [
        {
  ...
  methods: {
    ...
    async onSubmit(evt) {
      ...
      let data = this.form;
      try {
        let res = await Faq.AddFaqCategory(data);
        if (res.data.success) {
          this.success = true;
          this.$notify({
            message: "success",
  ...
  async created() {
    ...
    let res_Faq = await Faq.GetActive();
    let options_floor = [
      { value: null, text: "Please select an floor", disabled: true }
    ];
    let options_Faq = [
      { value: null, text: "Please select an Faq", disabled: true }
    ];
    ...
    res_Faq.data.data.map(data => {
      data = {
        value: data._id,
        text: `${data.name}`
      };
      options_Faq.push(data);
    });

    this.options_floor = options_floor;

    this.options_Faq = options_Faq;
