        ...
        <div class="row justify-content-center">
          <div class="col-6">
            <button
              ...
              @click="$router.push({ path: 'faqcategory/add' })"
            >
              <i class="nc-icon nc-simple-add"> New Faq Category</i>
            </button>
          </div>
        </div>
      </b-col>
...
export default {
  ...
  methods: {
    ...
    async handleOk(id) {
      ...
      try {
        let res = await Faq.DeleteFaqCategory(id);
        if (res.data.success) {
          this.success = true;
          this.notifyVue();
          this.loadStart();
    ...
    async loadStart() {
      try {
        this.isLoading = true;
        let res = await Faq.GetFaqCategory();
